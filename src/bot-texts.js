...
const CHATS = {
  ...
  notifyDualMode: 'Режим: сообщения из MAX дублируются в личные сообщения и в группу.',
  notifyDmMode: 'Режим: уведомления только в личные сообщения.',
  bindGroupPrompt: [
    'Нажмите кнопку ниже и выберите группу в Telegram.',
    'Бот должен быть участником группы.',
    '',
    'После привязки каждое сообщение из MAX будет приходить и вам в ЛС, и в эту группу.',
  ].join('\n'),
  notifyFooter: [
    'Один чат MAX → сразу в личку и в группу (если группа привязана).',
    '',
    '✅ справа — бот админ в группе, ❌ — выдайте права администратора.',
    'Когда бот станет админом, название группы подтянется само.',
    '',
    `Привязать группу: кнопка «${BUTTONS.bindGroup}» ниже.`,
    `Или: «${BUTTONS.discoverId}» → выберите чат → «${BUTTONS.bindNotify}».`,
  ].join('\n'),
  bound: {
    title: 'Чат привязан',
    lines: (isGroup) => [
      isGroup
        ? 'Уведомления из MAX будут приходить в личные сообщения и в эту группу. Сделайте бота администратором — название подтянется само.'
  ...
  addPrompt: [
    ...
    'Нажмите кнопку с названием чата или отправьте название / ссылку вручную, например:',
    '<code>https://web.max.ru/-999999999999</code>',
    '',
    'Отмена: /cancel',
  ].join('\n'),
  ...
  primary: '⭐ Основной — сюда идут ответы и действия по умолчанию.',
  secondary: 'Дополнительный чат. «Сделать основным» — чтобы ответы шли сюда.',
