...
const BUTTONS = {
  ...
  bindUser: 'Добавить пользователя',
  pickUser: 'Выбрать пользователя',
  userSettings: 'Настройки',
  userCanReply: 'Ответы в боте',
...
const HINTS = {
  ...
  profileBioTemplate: [
    ...
    '',
    'Переменные:',
    '<code>{час}</code> <code>{минута}</code> <code>{день}</code> <code>{месяц}</code>',
    '<code>{температура}</code> <code>{погода}</code>',
...
const CHATS = {
  discoverEmpty: [
    '<b>Чаты Telegram</b>',
    '',
    'Пока нет известных чатов. Добавьте группу через «Чаты MAX» → карточку чата.',
  ].join('\n'),
  discoverHint: [
  ...
  infoFooter: [
    ...
    'Личные сообщения получают уведомления всегда, если для чата MAX включено ЛС. Группы и других пользователей из этого списка можно выбрать отдельно у каждого чата MAX.',
  ].join('\n'),
  notifyHeader: 'Куда приходят уведомления',
  notifyEmpty: 'Чаты для уведомлений пока не выбраны.',
  notifyDualMode: 'Режим: ниже — группы и пользователи, куда бот умеет писать. Куда слать каждый чат MAX, выбирается в «Чаты MAX».',
  notifyDmMode: 'Режим: уведомления только в личные сообщения, пока не добавите группу или пользователя.',
  bindGroupPrompt: [
  ...
  bindUserPrompt: [
    'Добавьте пользователя, которому слать уведомления из MAX.',
    '',
    'Пришлите ссылку:',
    '• <code>https://t.me/username</code>',
    '• <code>@username</code>',
    '• <code>tg://user?id=123456789</code>',
    '',
    'Или нажмите «Выбрать пользователя» внизу.',
    '✅ — уже писал боту, ❌ — ещё нет. Пока человек не откроет бота, Telegram не даст написать ему в ЛС.',
    '',
    'В настройках пользователя можно разрешить ему отвечать в MAX кнопкой «Ответить».',
    '',
    'Отмена: /cancel',
  ].join('\n'),
  notAdmin: {
  ...
  notifyFooter: [
    'Это общий список чатов Telegram. Для каждого чата MAX в «Чаты MAX» можно включить ЛС, пользователей и несколько групп.',
    '',
    '✅ у группы — бот админ, ❌ — выдайте права администратора.',
    '✅ у пользователя — он уже писал боту, ❌ — ещё нет.',
    '«Удалить» убирает чат из рассылки. «Добавить группу» приглашает бота с правами администратора. «Добавить пользователя» — по ссылке или выбору.',
  ].join('\n'),
  bound: {
    title: 'Чат привязан',
    lines: (isGroup) => [
      isGroup
        ? 'Группа добавлена в список. В «Чаты MAX» отметьте, из каких чатов MAX отправлять сюда сообщения.'
        : 'Пользователь добавлен в список. В «Чаты MAX» отметьте, из каких чатов MAX отправлять ему сообщения.',
    ],
  },
  ...
  addPrompt: [
    ...
    'На кнопках — ссылки https://web.max.ru/… из этого списка. Личные чаты сразу уходят в ЛС; для групп можно выбрать ЛС, пользователей и нужные группы Telegram.',
  ...
  addPromptNoScreenshot: [
    '<b>Добавить чат MAX</b>',
    '',
    'На кнопках — ссылки https://web.max.ru/… из списка слева. Личные чаты сразу уходят в ЛС; для групп можно выбрать ЛС, пользователей и нужные группы Telegram.',
    'Если чатов много — листайте список кнопками ◀️ ▶️.',
    'Можно отправить название или ссылку вручную, например:',
    '<code>https://web.max.ru/-999999999999</code>',
    '',
  ...
  notifyTargetGroup: 'Куда слать: только в выбранные группы.',
  notifyTargetBoth: 'Куда слать: в личные сообщения, пользователей и выбранные группы.',
  notifyDestHint:
    'Отметьте, куда слать сообщения из этого чата MAX: ЛС, пользователи и группы. Пользователя можно добавить по ссылке. У пользователя — «Настройки», чтобы разрешить ответы в боте.',
  notifyDestNone:
    'Сейчас некуда отправлять — отметьте ЛС, пользователя или группу ниже. Новую группу или пользователя добавьте кнопками внизу.',
  notifyDestWhereTitle: 'Куда слать в Telegram',
  notifyDestWhereHint: 'Отметьте ЛС, пользователей и нужные группы, затем нажмите «Готово». Можно несколько получателей или только личка. У дополнительного пользователя кнопка «Настройки» — можно разрешить ответы в MAX.',
  userReplyHint:
    'Если включить «Ответы в боте», у этого пользователя появится кнопка «Ответить» на уведомлениях, и он сможет писать в MAX.',
  userReplyOn: 'Ответы в боте: ✅ можно',
  userReplyOff: 'Ответы в боте: ❌ нельзя',
};
